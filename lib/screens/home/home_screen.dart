  ...
  Future<void> _scanDocument(BuildContext context, ScannerFileSource source) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Permission required to access the scanner.")));
    }
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Scanner failed. Please try again.")));
      }
    }
  }
